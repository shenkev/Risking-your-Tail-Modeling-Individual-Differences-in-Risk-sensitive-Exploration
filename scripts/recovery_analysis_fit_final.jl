...
ground_truth = parsefile("./out/recovery/abcsmc_best_samples.json")
...
if animalᵢ in timid_animals
    ...
    animal_group = "timid"
    xₛ, dₚ, dₛ, fᵣ = ground_truth["x"]["$animalᵢ"]
...
elseif animalᵢ in intermediate_animals
    ...
    x₂, _, d₁, d₂, fᵣ = ground_truth["x"]["$animalᵢ"]
    ...
    x₂, x₃, d₁, d₂, d₃, fᵣ = ground_truth["x"]["$animalᵢ"]

    S₀ = [x₂, x₃, d₁, d₂, d₃, fᵣ]  # freq ratio of steady-state/peak < 1.0
...
run(p, h, ϵ₀, "./out/recovery_final/animal$animalᵢ.jls")
